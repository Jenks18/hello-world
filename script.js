...
function selectCourse(course) {
    ...
    createCourseTabs();
    displayCourseHoleInfo(course);
    setActiveTab(document.querySelector('#course-tabs .tab'));
}
...
function displayCourseHoleInfo(course) {
    ...
    course.holes.forEach(hole => {
        ...
        row.innerHTML = `
            ...
            <td>${hole.par}</td>
            <td>${hole.length}m</td>
            <td>${hole.tee} &rarr; ${hole.target}</td>
            <td>${hole.avg}</td>
            <td>${hole.scoreDistribution}</td>
        `;
        tbody.appendChild(row);
    });
    holeDetail.appendChild(table);
}
